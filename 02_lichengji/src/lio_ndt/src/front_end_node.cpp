...
int main(int argc, char *argv[])
{
    ...
    // 前端 - 使用new而不是make_shared来避免Eigen内存对齐问题
    std::shared_ptr<FrontEnd> front_end_ptr(new FrontEnd());
    
    // 强制使用NDT配准方法
    front_end_ptr->SetRegistrationMethod(FrontEnd::NDT);
    ROS_INFO("Using NDT registration method (forced)");
            ...
            while (cloud_data_buff.size() > 0 && imu_data_buff.size() > 0 && gnss_data_buff.size() > 0)
            {
                ...
                else
                {
                    ...
                    if ((int)run_time % 10 == 0) {
                        ROS_INFO("Current run_time: %.2f seconds", run_time);
                    }
                    // 每5秒输出一次运行时间用于调试
                    static double last_debug_time = 0;
                    if (run_time - last_debug_time >= 5.0) {
                        ROS_INFO("Debug: Current run_time: %.2f seconds, global map size: %zu frames", 
                                run_time, front_end_ptr->GetGlobalMapSize());
                        last_debug_time = run_time;
                    }
                }
                ...
                // 定期发布全局地图并保存 - 基于帧数而不是时间
                static int frame_count = 0;
                frame_count++;
                
                if (frame_count % 50 == 0) { // 每50帧输出一次状态
                    ROS_INFO("Processed %d frames, global map size: %zu frames", 
                            frame_count, front_end_ptr->GetGlobalMapSize());
                }
                
                // 当处理了200帧或全局地图有100帧时保存
                if ((frame_count >= 200 || front_end_ptr->GetGlobalMapSize() >= 100) && !has_global_map_published)
                {
                    ROS_INFO("Attempting to save global map, processed frames: %d, global map size: %zu", 
                            frame_count, front_end_ptr->GetGlobalMapSize());
                    
                    // 如果全局地图有数据，直接保存
                    if (front_end_ptr->GetGlobalMapSize() > 0)
                    {
                        ROS_INFO("Global map has %zu frames, proceeding to save...", front_end_ptr->GetGlobalMapSize());
                        
                        // 尝试获取全局地图用于发布
                        if (front_end_ptr->GetNewGlobalMap(global_map_ptr)) 
                        {
                            global_map_pub_ptr->Publish(global_map_ptr);
                            ROS_INFO("Global map published to ROS topic");
                        }
                        
                        // 直接调用SaveGlobalMap保存到文件
                        std::string map_save_path = WORK_SPACE_PATH + "/slam_data/map/global_map.pcd";
                        front_end_ptr->SaveGlobalMap(map_save_path);
                        ROS_INFO("Global map save attempt completed!");
                        
                        // 保存NDT配准结果
                        std::string ndt_result_save_path = WORK_SPACE_PATH + "/slam_data/map/ndt_registration_result.pcd";
                        front_end_ptr->SaveNDTRegistrationResult(ndt_result_save_path);
                        ROS_INFO("NDT registration result save attempt completed!");
                        
                        has_global_map_published = true;
                    }
                    else
                    {
                        ROS_WARN("Global map is empty, cannot save yet");
                    }
                }
            }
        }
